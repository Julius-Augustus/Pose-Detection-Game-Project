...
class DetectionVisualizer:
    # 预定义全身关键点连接关系
    ALL_BODY_POSE_CONNECTIONS = mp.solutions.pose.POSE_CONNECTIONS
    ...
    def draw_landmarks(cls, frame, landmarks, show_index=False, line_thickness=2):
        ...
        # 全身关键点索引
        ALL_BODY_LANDMARKS = list(range(33))
        all_body_landmarks = [landmarks[i] for i in ALL_BODY_LANDMARKS if i < len(landmarks)]

        # 绘制连接线
        for connection in cls.ALL_BODY_POSE_CONNECTIONS:
            start_idx, end_idx = connection
            start_point = cls._get_pixel_coord(all_body_landmarks[ALL_BODY_LANDMARKS.index(start_idx)], frame_width, frame_height)
            end_point = cls._get_pixel_coord(all_body_landmarks[ALL_BODY_LANDMARKS.index(end_idx)], frame_width, frame_height)

            if start_point and end_point:
                cv2.line(frame, start_point, end_point, cls.CONNECTION_COLOR, line_thickness)

        # 绘制关键点
        for idx, landmark in enumerate(all_body_landmarks):
            ...
            if center:
                ...
                # 显示索引编号
                if show_index:
                    cv2.putText(frame, str(ALL_BODY_LANDMARKS[idx]),
